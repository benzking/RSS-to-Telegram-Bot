...
def send(chatid, xml, feed_title, url, context):
    for _ in range(2):
        try:
            message=send_message(chatid, xml, feed_title, url, context)
            break
...
def send_message(chatid, xml, feed_title, url, context):
    ...
    if video:
        ...
        message=send_media_message(chatid, xml, feed_title, url, video, context)
        return  message
        # for weibo, only 1 video can be attached, without any other pics

    if pics:
        print('\t\t- Detected pic(s), ', end="")
        if pics:
            print('send pic(s) message(s).')
            message=send_media_message(chatid, xml, feed_title, url, pics, context)
            return message
        else:
            print('but too large.')

    print('\t\t- No media, send text message(s).')
    message=send_text_message(chatid, xml, feed_title, url, False, context)
    return  message
...
def send_media_message(chatid, xml, feed_title, url, media, context):
    ...
    if type(media) == str:  # TODO: just a temporary workaround
        message=context.bot.send_video(chatid, media, caption=head + text_list[0], parse_mode='MarkdownV2',
        ...
        message= context.bot.send_media_group(chatid, pic_objs)
        print(f'\t\t\t- {len(pic_objs)} pics message.')
        return message

    if number > 1:
        message = send_text_message(chatid, text_list, feed_title, url, True, context)
        return message
